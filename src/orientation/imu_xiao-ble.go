...
type IMU struct {
	...
	gyrCal *GyrCal
	buf    [2]byte // buffer for reading tap source register, having it here avoids heap allocation
}
...
func (imu *IMU) ReadTap() (tap bool) {
	imu.buf[0] = TAP_SRC
	imu.buf[1] = 0x00
	machine.I2C1.Tx(imu.device.Address, imu.buf[0:1], imu.buf[1:2])
	return imu.buf[1]&0x10 != 0
}
